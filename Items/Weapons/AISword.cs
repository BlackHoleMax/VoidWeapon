...
namespace VoidWeapon.Items.Weapons
{
    ...
    public class AISword : ModItem
    {
        ...
        private int shotCount = 0; // 发射计数器，用于控制特殊弹幕的发射频率
        ...
        public override void SetDefaults()
        {
            ...
            // 弹幕设置
            Item.shoot = ModContent.ProjectileType<Projectiles.AISwordQi>();
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            if (shotCount % SpecialShotInterval == 0)
            {
                ...
                Projectile.NewProjectile(source, shootPosition, shootVelocity, 
                    ModContent.ProjectileType<Projectiles.AISwordQiChange>(), 
                    (int)(damage * 1.3f), knockback * 1.1f, player.whoAmI, 1f, 0f);
            }
            else
            {
                // 发射普通AISwordQi弹幕
                Projectile.NewProjectile(source, shootPosition, shootVelocity, type, damage, knockback, player.whoAmI, 0f, 0f);
            }
