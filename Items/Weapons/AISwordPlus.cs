...
namespace VoidWeapon.Items.Weapons
{
    public class AISwordPlus : ModItem
    {
        ...
        // 强化攻击剩余次数
        private int empoweredAttacksLeft = 0;
        ...
        public override void SetDefaults()
        {
            ...
            // 弹幕设置
            Item.shoot = ModContent.ProjectileType<Projectiles.AISwordPlusQi>();
        ...
        public override void HoldItem(Player player)
        {
            ...
            if (player.controlUseTile && cooldownTimer <= 0) // 右键按住且不在冷却中
            {
                ...
                {
                    dashPlayer.StartRightClickCharging();
                }
                ...
                if (chargeProgress > 0)
                {
                    ...
                    if (Main.rand.NextBool(3))
                    {
                        ...
                        Color color = Color.Lerp(Color.Purple, Color.Red, chargeProgress);
                        Dust dust = Dust.NewDustDirect(playerCenter, 0, 0, DustID.Shadowflame,
                    ...
                    {
                        Vector2 playerCenter = player.Center;
                        Color rainbowColor = Main.hslToRgb((Main.GameUpdateCount * 0.05f + chargeProgress) % 1f, 1f, 0.8f);
                        Dust dust = Dust.NewDustDirect(playerCenter, 0, 0, DustID.RainbowMk2,
                    ...
                    {
                        Vector2 playerCenter = player.Center;
                        Color rainbowColor = Main.hslToRgb((Main.GameUpdateCount * 0.1f) % 1f, 1f, 1f);
                        Dust dust = Dust.NewDustDirect(playerCenter, 0, 0, DustID.RainbowMk2,
            ...
            else
            {
                ...
                if (dashPlayer.isRightClickCharging)
                {
                    dashPlayer.StopRightClickCharging();
                }
            }
        ...
        public override void UseItemHitbox(Player player, ref Rectangle hitbox, ref bool noHitbox)
        {
            // 获取蓄力进度
            DashPlayer dashPlayer = player.GetModPlayer<DashPlayer>();
            float chargeProgress = dashPlayer.GetLeftClickChargeProgress();

            // 根据蓄力程度增加攻击范围
            if (chargeProgress > 0.5f)
            {
                int expand = (int)(10 * chargeProgress);
                hitbox.X -= expand;
                hitbox.Y -= expand;
        ...
        public override void ModifyWeaponDamage(Player player, ref StatModifier damage)
        {
            ...
            if (chargeProgress > 0)
            {
                damage *= 1f + (chargeProgress * 1.5f); // 最多增加150%伤害
            }
        }
        ...
        public override void ModifyHitNPC(Player player, NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            if (chargeProgress > 0)
            {
                modifiers.Knockback *= 1f + (chargeProgress * 1f); // 最多增加100%击退
            }
        }
        ...
        {
            // 向四面八方发射弹幕
            ExecuteDashAttack(Main.LocalPlayer, Main.LocalPlayer.GetSource_OnHit(target), target.Center, hit.Damage, hit.Knockback);
        }
        ...
        private void ExecuteDashAttack(Player player, IEntitySource source, Vector2 position, int damage, float knockback)
        {
            ...
            for (int i = 0; i < projectileCount; i++)
            {
                ...
                Projectile.NewProjectile(source, spawnPosition, velocity,
                    ModContent.ProjectileType<Projectiles.AISwordPlusQi>(),
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                Dust dust = Dust.NewDustDirect(position, 0, 0,
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            // 正常攻击
            Vector2 shootPosition = player.RotatedRelativePoint(player.MountedCenter, true);
            Vector2 shootVelocity = velocity.SafeNormalize(Vector2.UnitX * player.direction) * Item.shootSpeed;
        ...
        private void ExecuteEmpoweredAttack(Player player, IEntitySource source, Vector2 position, int damage, float knockback)
        {
            // 向四面八方发射弹幕，总共16个
            int projectileCount = 16;
            float speed = 8f; // 弹幕速度
        ...
        public override void AddRecipes()
        {
            ...
            recipe.AddIngredient(ModContent.ItemType<AISword>(), 1); // 需要虚空追踪者
        ...
        public override void MeleeEffects(Player player, Rectangle hitbox)
        {
            ...
            {
                // 彩虹色粒子效果
                Color rainbowColor = Main.hslToRgb((Main.GameUpdateCount * 0.02f) % 1f, 1f, 0.7f);
            ...
            if (Main.rand.NextBool(3))
            {
                Dust voidDust = Dust.NewDustDirect(new Vector2(hitbox.X, hitbox.Y), hitbox.Width, hitbox.Height,
            ...
            if (isEmpowered)
            {
                Dust empoweredDust = Dust.NewDustDirect(new Vector2(hitbox.X, hitbox.Y), hitbox.Width, hitbox.Height,
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            ...
            // 添加特殊功能说明
            TooltipLine specialAbilityLine = new TooltipLine(Mod, "SpecialAbility", "右键蓄力3秒后自动激活强化：回复200生命值，接下来三次攻击向四面八方发射弹幕");
            specialAbilityLine.OverrideColor = Color.HotPink;
            tooltips.Add(specialAbilityLine);

            // 添加冷却时间说明
            TooltipLine cooldownLine = new TooltipLine(Mod, "Cooldown", "强化冷却时间: 3秒");
