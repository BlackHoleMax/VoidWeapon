...
public class AISwordPlusQi1 : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        var initialSpeed = 12f; // 与在AISwordPlusQi.cs中设置的速度一致

        // 在第一帧时保存初始速度大小
        if (Projectile.ai[0] == 0) Projectile.ai[0] = initialSpeed;
        ...
        {
            var rainbowColor = Main.hslToRgb(Main.GameUpdateCount * 0.03f % 1f, 1f, 0.6f);
            var dustPos = Projectile.Center + Main.rand.NextVector2Circular(20f, 20f);
            var dust = Dust.NewDustDirect(dustPos, 0, 0, DustID.RainbowMk2, 0f, 0f, 100, rainbowColor, 1.2f);
        ...
        if (Main.rand.NextBool(4))
        {
            var voidDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height,
                DustID.Shadowflame, 0f, 0f, 100, Color.Purple);
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        for (var i = 0; i < 15; i++)
        {
            var rainbowColor = Main.hslToRgb((Main.GameUpdateCount * 0.02f + i * 0.15f) % 1f, 1f, 0.7f);
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        for (var i = 0; i < 12; i++)
        {
            var rainbowColor = Main.hslToRgb(i * 0.2f % 1f, 1f, 0.7f);
