...
public class AISwordPlusQi : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        // 自动追踪逻辑
        var homingRange = 600f; // 追踪范围
        var homingSpeed = 0.2f; // 追踪速度
        ...
        if (targetNPC != null)
        {
            ...
            {
                var rainbowColor = Main.hslToRgb(Main.GameUpdateCount * 0.03f % 1f, 1f, 0.8f);
                var trackDust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.RainbowMk2,
        ...
        for (var i = 0; i < 4; i++)
        {
            var rainbowColor = Main.hslToRgb((Main.GameUpdateCount + i * 15) * 0.02f % 1f, 1f, 0.7f);
            var dustPos = Projectile.position +
                          Main.rand.NextVector2Circular(Projectile.width * 0.5f, Projectile.height * 0.5f);
            var dust = Dust.NewDustDirect(dustPos, 0, 0, DustID.RainbowMk2, 0f, 0f, 100, rainbowColor, 1.8f);
        ...
        {
            var voidDust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height,
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        {
            var spawnPos = targetCenter + directions[i] * 80f; // 距离敌怪80像素
            var velocity = directions[i] * 12f; // 朝敌怪方向射击
        ...
        for (var i = 0; i < 15; i++)
        {
            var voidDust = Dust.NewDustDirect(target.Center, 0, 0,
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        for (var i = 0; i < 30; i++)
        {
            var rainbowColor = Main.hslToRgb(i * 0.1f % 1f, 1f, 0.8f);
        ...
        for (var i = 0; i < 20; i++)
        {
            var voidDust = Dust.NewDustDirect(Projectile.Center, 0, 0,
